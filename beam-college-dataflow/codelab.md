...
* user-provided, e.g., structured formats like JSON and CSV where user need to specify the schema for converting into 
Beam Rows. 
...
In the 
`/transforms/io/GcsIO.java` find `FORMAT` enum, that lists all supported formats.
```java
public enum FORMAT {
    JSON,
    CSV,
